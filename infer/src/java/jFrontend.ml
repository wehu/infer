...
let create_icfg linereader program icfg cn node =
  ...
  begin
    Javalib.m_iter (fun m ->
			try
			  JTrans.create_local_procdesc program linereader cfg tenv node m
      (* wehu ignore any exception since the classes generated by scalac maybe not standard *)
			with e -> JUtils.log "oops %s@." (Printexc.to_string e)) node;
    Javalib.m_iter (fun m ->
			try
